...
A plug-and-play security scanning workflow powered by [`SECURITY.md`](./SECURITY.md). Point your LLM agent at this file and it will automatically install and run three industry-standard security tools against any codebase — no manual setup required.
...
1. **Detect your operating system** (Ubuntu or macOS)
2. **Check whether each security tool is installed** — and install any that are missing
3. **Run a full security scan** using three tools in sequence
4. **Summarise all findings** with severity ratings, file locations, and remediation advice

### Tools Used

| Tool | Purpose |
|---|---|
| [Semgrep](https://semgrep.dev) | Static code analysis — OWASP Top 10, security audit rules, hardcoded secrets in source |
| [Trivy](https://trivy.dev) | Dependency CVEs, IaC misconfigurations (Dockerfile, Terraform, Kubernetes), secrets |
| [Gitleaks](https://github.com/gitleaks/gitleaks) | Secrets and credentials leaked in Git history or staged changes |
...
1. Download or copy [`SECURITY.md`](./SECURITY.md) from this repository
2. Place it in the root of your project:
   ...
   └── ...
   ```
3. Open your LLM of choice and use the prompt in the section below
...
| File | Contents |
|---|---|
| `semgrep-results.json` | OWASP Top 10 and security audit findings |
| `trivy-results.json` | CVEs, misconfigurations, and secrets |
| `gitleaks-results.json` | Secrets detected in full Git history |
| `gitleaks-staged-results.json` | Secrets detected in staged changes |
...
MIT — free to use, copy, and adapt in your own projects.
